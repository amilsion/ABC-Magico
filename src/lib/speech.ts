...
function pickVoice(): SpeechSynthesisVoice | null {
  ...
  const ptBR =
    voices.find((v: SpeechSynthesisVoice) => v.lang === 'pt-BR' && /female|mulher|luciana/i.test(v.name)) ||
    voices.find((v: SpeechSynthesisVoice) => v.lang === 'pt-BR') ||
...
export function playSound(kind: EffectKind) {
  ...
  const config: Record<EffectKind, { freq: number; type: OscillatorType; dur: number; sweep?: number }> = {
    correct: { freq: 523.25, type: 'sine', dur: 0.18, sweep: 880 },
    wrong: { freq: 220, type: 'sawtooth', dur: 0.25, sweep: 110 },
    win: { freq: 523.25, type: 'triangle', dur: 0.45, sweep: 1046.5 },
